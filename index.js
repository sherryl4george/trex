...
(function () {
    ...
    Runner.prototype = {
        ...
        init: function () {
			...
			// Additional Stuff for NN
			this.GA = new GeneticAlgorithm(10,4);
			this.GA.reset();
			this.maxPop = this.GA.max_units;
        ...
        onKeyDown: function (e) {
            ...
            // if (e.target != this.detailsButton) {
                if (!this.crashed && (Runner.keycodes.JUMP[e.which])) {
				//||
            ...
            if (this.playing && !this.crashed && Runner.keycodes.DUCK[e.which]) {
                e.preventDefault();
                if (this.tRex.jumping) {
        ...
        gameOver: function () {
            ...
            // } else {
                // this.gameOverPanel.draw();
            // }
			this.GA.Population[this.currentPop-1].fitness = this.distanceMeter.getActualDistance(Math.ceil(this.distanceRan)) - this.distanceMeter.getActualDistance((Math.abs( this.tRex.xPos - this.horizon.obstacles[0].xPos )));
			this.GA.Population[this.currentPop-1].score = this.horizon.obstacleCount;
			// if(this.horizon.obstacleCount>0) console.log("ddddd");
		...
		checkAction : function(){
			if(!this.crashed){
			var outputs = this.GA.activateBrain(this.tRex, this.horizon.obstacles[0], this.currentPop, this.currentSpeed);
			// console.log(outputs);
				if(outputs[0] > 0.5){
					// For jump Up
				...
				// else if(outputs[0] > 0.35 && outputs[0] < 0.5){
					// var e = jQuery.Event("keydown");			
					// e.which = 40; // it is down
					// this.onKeyDown(e);
					// // setTimeout(this.onKeyUp(e), 2000);
				// }
				// if(outputs[1] > 0.5){
					// // For jump Up
					// var e = jQuery.Event("keydown");			
					// e.which = 40; // it is down
					// this.onKeyDown(e);
					// // setTimeout(this.onKeyUp(e), 1000);
				// }
			}
			//if(this.horizon.obstacles.length > 2 ){
		...
		startNN : function(){			
			...
			e.which = 38; // it is down
			this.onKeyDown(e);*/
			if(this.GA.iteration == 1 && this.currentPop == 10 ){
				this.GA.reset();
			...
			if(this.currentPop == 10 ){
				console.log("Generation: "+this.GA.iteration);
				console.log("Iteration: "+this.GA.iteration+" : "+ this.currentPop);
			}
			else if(this.currentPop < 10 && this.currentPop > 0){
				if(this.crashed){
					var e = jQuery.Event("keydown");			
					e.which = 13; // it is down
					this.onKeyUp(e);
					console.log("Iteration: "+this.GA.iteration+" : "+ this.currentPop);
				}
			}
				...
				this.GA.iteration++;
				this.currentPop = this.GA.max_units;
				var e = jQuery.Event("keydown");			
				e.which = 13; // it is down
				this.onKeyUp(e);
				//console.log(this.GA.population);
    ...
    Obstacle.types = [
        ...
        },
        {
            ...
            width: 46,
            height: 40,
            yPos: [75, 50], // Variable height. //NN Stuff
...
GeneticAlgorithm.prototype = {
	...
	activateBrain : function(tRex, obstacle, index, speed){		
	/*
		// input 1: the horizontal distance between the bird and the target
		var targetDeltaX = this.normalize(obstacle.xPos + obstacle.width, 600) * this.SCALE_FACTOR;
		...
		var targetDeltaY = this.normalize(tRex.xPos - obstacle.xPos + obstacle.width, 600) * this.SCALE_FACTOR;
		
		//input 3: Current Speed of tRexBox
		var tRexSpeed = this.normalize(speed, 13) * 5;
		
		//input 4: Obstacle yPos
		var oWidth = this.normalize(obstacle.width , 30 ) * 100;
	
		// create an array of all inputs
		var inputs = [targetDeltaX, targetDeltaY, tRexSpeed];*/
		...
		var outputs = this.Population[index-1].activate(inputs);
	...
	// activateBrain : function(tRex, obstacle, index, speed){		
		// // input 1: the horizontal distance between the bird and the target
		// var targetDeltaX = this.normalize(obstacle.width, 100) * this.SCALE_FACTOR; //Position of Obstacle
		// var targetDeltaX1 = this.normalize(obstacle.xPos, 600) * this.SCALE_FACTOR; // Width of Obstacle
		// // var gapObstacle = this.normalize(obstacle.xPos - this.horizon.obstacles[1].xPos, 200) * this.SCALE_FACTOR; // Width of Obstacle
		// // var targetDeltaX = this.normalize(obstacle.xPos , 600) * this.SCALE_FACTOR;
		...
		// //input 4: Obstacle yPos
		// var oYPos = this.normalize(1/obstacle.yPos , 5 ) * 200;
	
		// // create an array of all inputs
		// var inputs = [targetDeltaX, targetDeltaX1, targetDeltaY, tRexSpeed, oYPos];
